...
def post_data():
    ...
    resume_name = '%s-%s_(%s).pdf' % (content['First'], content['Last'], issue_key)
    ...
    #  attach resume to issue corresponding to 'issue_key'
    attachment_url = 'http://testjira.transmarketgroup.com:8080/rest/api/2/issue/%s/attachments' % issue_key
    subprocess.call(['curl','-D-','-u','tommy.barnes:password','-X','POST','-H',"X-Atlassian-Token: nocheck",'-F',"file=@%s" % resume_name, attachment_url])
    subprocess.call(['rm','%s' % resume_name])
